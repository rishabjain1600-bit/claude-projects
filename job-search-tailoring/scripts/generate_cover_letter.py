#!/usr/bin/env python3
"""
generate_cover_letter.py — Render a tailored cover letter PDF from JSON data.

Usage:
    python3 scripts/generate_cover_letter.py --data output/Stripe_coverletter_data.json
    python3 scripts/generate_cover_letter.py --data output/Stripe_coverletter_data.json --output output/
"""
...
def render_pdf(data: dict, template_dir: Path, output_dir: Path) -> Path:
    ...
    output_path = output_dir / f"{company}_CoverLetter.pdf"

    doc = weasyprint.HTML(string=html_content, base_url=str(template_dir)).render()

    page_count = len(doc.pages)
    if page_count != 1:
        print(f"\nERROR: Cover letter rendered to {page_count} page(s) — must be exactly 1 page.")
        print("Fix: trim a sentence from the longest paragraph and re-run.\n")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--data", required=True, help="Path to cover letter data JSON file")
    ...
    print(f"Rendering cover letter for: {data.get('company', 'Unknown')}")
